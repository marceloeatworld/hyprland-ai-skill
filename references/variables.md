<!-- source: https://wiki.hypr.land/configuring/core/config-options -->
...
| nogroup_border_active | Border color for the active window that cannot be added to a group | gradient | `0xffff00ff` | |
...
| lockdead_screen_delay | Delay in ms after which the "lockdead" screen will appear in case a locks creen app fails to cover all the outputs | int | `1000` | [0 - 5000] |
...
| damage_tracking | Redraw only the needed bits of the display. Do **not** change. None - 0, monitor - 1, full - 2 | int | `2` | [0 - 2] |
