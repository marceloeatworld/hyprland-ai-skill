<!-- source: https://wiki.hypr.land/hyprland-plugins -->
...
This section describes the process of developing plugins for Hyprland.
It is intended to be read by developers, not end users.
...
Plugins are basically dynamic objects loaded by Hyprland.
They have (almost) full access to every part of Hyprland's internal process, and as such, can modify and change way more than a script.
...
If you install with `make install`, you should have the headers.
...
Run `cd Hyprland && make debug && sudo make installheaders && cd ..`.
...
Feel free to take a look at the header.
It contains a bunch of useful comments.
...
We will initialize it in our plugin init function later.
It serves as an internal "ID" of our plugin.
...
Do NOT change it.
It will be set to the correct value when compiling.
...
The first method will be called when your plugin gets initialized (loaded), and is thus _required_.
...
It's worth noting that adding config variables are _only_ allowed in this function.
...
You do not have to unload layouts, remove config options, remove dispatchers, window decorations or unregister hooks in the exit method.
Hyprland will do that for you.

### Setting up a development environment

In order to make your life easier, it's a good idea to work on a nested debug Hyprland session.
Unless you need to test some things that require real hardware (e.g. trackpad gestures), you definitely should use a nest.

See [the Contributing Section](../../../contributing-and-debugging/#development-environment) for instructions on setting up a development environment.
...
`name` and `authors` are required.
`commit_pins` are optional.
See [commit pins](#commit-pins) for more info.
...
`description`, `authors` are optional.
`output` and `build` are required.

`build` are the commands that `hyprpm` will run in the root of the repo to build the plugin.
Every command will reset the current working directory to the repo root.

`output` is the path to the output `.so` file from the root of the repo.

### Commit pins

Commit pins allow you to manage versioning of your plugin.
...
Function hooks allow your plugin to intercept all calls to a function of your choice.
They are to be treated as a last resort, as they are the easiest thing to break between updates.
