<!-- source: https://wiki.hypr.land/configuring/extra/systemd -->
...
This previously required manual setup, but is now integrated into Hyprland and handled automatically.
If you have a custom `hyprland-session.target` taken from a previous version of this article, it can be cleaned up via `systemctl --user revert hyprland-session.target`.
Also, any leftover `systemctl` commands in your config, such as the following, should be *removed*:
...
> You may want to adopt some variation of the lua event listeners above in order to make your sessions behave as desired.
