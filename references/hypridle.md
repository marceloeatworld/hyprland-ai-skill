<!-- source: https://wiki.hypr.land/hypr-ecosystem/user/hypridle -->
...
A listener can gate its `on-timeout` on external state with `condition_cmd`.
When the timeout is reached, hypridle runs `condition_cmd` first; `on-timeout` fires only if it exits `0`.
A non-zero exit defers `on-timeout`.

Set `condition_retry` to keep re-checking while the user stays idle: hypridle re-runs `condition_cmd` every `condition_retry` seconds and fires `on-timeout` the moment it succeeds.
With `condition_retry = 0` (the default), a deferred `on-timeout` is simply skipped until the next idle cycle.
